...
#define LED2 PC5            //druha ledka
#define BUTTON PD7        //tlacitko
...
#include "gpio.h"           // GPIO library for AVR-GCC, proc v uvozovkach? protoze to nacitame LOKALNE.
...
int main2(void)
{
    ...
    while (1)
    {
        ...
        if ((GPIO_read(&PINC, BUTTON)) == 1)
            {
                GPIO_toggle(&DDRB, LED_GREEN);
                GPIO_toggle(&DDRC, LED2);
            }
